...
import { StatusBar, Dimensions, TouchableOpacity, Text } from 'react-native';
...
const DeleteAll = styled.TouchableOpacity`
    font-size: 30px;
    font-weight: 600;
    color: ${({theme})=>theme.text};
    align-self: center;
...
export default function App() {
  ...
  const _deleteCompletedTasks = () => {
    ...
    for (let i = 0; i < Object.keys(currentTasks).length; i++){
      if (currentTasks[Object.keys(currentTasks)[i]].completed == true) {
        // console.log(Object.keys(currentTasks));
        tmpContainer.push([Object.keys(currentTasks)[i]]);
      }
    }
    // console.log(tmpContainer);
    for (let i = 0; i < tmpContainer.length; i++){
      delete currentTasks[tmpContainer[i]];
  ...
  (
    ...
    <ThemeProvider theme={theme}>
      <Container>
        ...
        <DeleteAll onPressOut = {_deleteCompletedTasks}>
          <DeleteAllText>완료항목 전체삭제</DeleteAllText>
          </DeleteAll>
      </Container>
    </ThemeProvider>
  );
